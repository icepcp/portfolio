<script lang="ts">
	...
	export let showOrg: boolean = false;
	...
	onMount(() => {
		activities = getActivities(activityType);
	});
...
<div class="mt-5 flex items-center gap-2 px-5">
	...
	<button type="button" on:click={() => (sortOrder = sortOrder === 1 ? -1 : 1)}>
			...
			<li class="relative mt-5 w-xs rounded-md bg-slate-100 p-3 shadow-sm ring ring-slate-300">
				<div class="flex gap-x-3">
					...
					<div>
						...
						{#if showOrg && activity.metadata.org}
